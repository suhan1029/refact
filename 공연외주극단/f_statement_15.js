...
// 함수 옮기기를 적용하여 amountFor(), volumeCreditsFor() 함수를 statement로 옮기기
...
function statement(invoice, plays) {
    ...
    function enrichPerformance(aPerformance) {
        ...
        result.amount = amountFor(result);                  // 2. 중간 데이터에 연극 정보를 저장
        result.volumeCredits = volumeCreditsFor(result);    // 2. 중간 데이터에 연극 정보를 저장
        return result;
    }
    ...
    function amountFor(aPerformance) {  // 1. 함수 옮기기로 옮겨옴
        let result = 0;
    ...
    function volumeCreditsFor(aPerformance) {  // 1. 함수 옮기기로 옮겨옴
        let result = 0;
...
function totalAmount() {
    let result = 0;
    for (let perf of data.performances) {
        result += perf.amount;  // 3. 중간 데이터를 사용하도록 바꾸기
    }
    return result;
}
...
function totalVolumeCredits() {
    let result = 0;
    for (let perf of data.performances) {
        result += perf.volumeCredits;  // 3. 중간 데이터를 사용하도록 바꾸기
    }
    return result;
}
